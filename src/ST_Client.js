...
module.exports = class ST_Client {
        ...
        sendDeviceCommand(devid, cmd, vals) {
                let that = this;
                ...
                return new Promise((resolve) => {
                            that.log.debug(`Sending Device Command: ${cmd}${(vals !== undefined ? ` | Value: ${JSON.stringify(vals)}` : "Nothing")} | DeviceID: (${devid}) | SendToLocalHub: (${sendLocal})`);
            rp(config)
                .then((body) => {
                    that.log('sendDeviceCommand Resp:', body);
                    resolve(body);
                })
                .catch((err) => {
