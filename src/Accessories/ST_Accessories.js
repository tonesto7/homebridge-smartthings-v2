...
function AccessoryDevice(platform, device) {
    // console.log("AccessoryDevice: ", platform, device);
    ...
    that.initializeDeviceCharacteristics(that, device, platform);

    that.loadData(device, that);
...
module.exports = function(oAccessory, oService, oCharacteristic, ouuid) {
    if (oAccessory) {
        ...
        inherits(AccessoryDevice, Accessory);
    }
    return AccessoryDevice;
};
module.exports.AccessoryDevice = AccessoryDevice;

AccessoryDevice.prototype = {
    ...
    CreateFromCachedAccessory: (accessory, platform) => {
        ...
        let firstLoadfn = (data, myObject) => {
            if (!this.device) {
                platform.log.debug("Performing first data load and characteristic initialization.");
                this.device = data;
