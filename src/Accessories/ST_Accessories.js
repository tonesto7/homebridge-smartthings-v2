...
module.exports = class ST_Accessories {
    constructor(platform) {
        ...
        this.CommunityTypes = require('../CommunityTypes')(this.Service, this.Characteristic);
    ...
    PopulateAccessory(accessory, deviceData) {
        // console.log("AccessoryDevice: ", platform, deviceData);
        ...
        this.getOrAddService = (svc) => {
            let s = accessory.getService(svc);
            if (!s) { s = accessory.addService(svc); }
            return s;
        };
        ...
        this.initializeDeviceCharacteristics(accessory, deviceData);

        this.loadData(deviceData, accessory);
    ...
    CreateFromCachedAccessory(accessory) {
        ...
        // accessory.services = this.getServices();

        // let loadDataFn = this.loadData.bind(accessory);
        // let firstLoadfn = (deviceData, myObject) => {
        // console.log(deviceData);
        //     console.log(myObject);
        if (!accessory.device) {
            accessory.device = deviceData;
            this.log.debug("Performing first data load and characteristic initialization.");
            this.initializeDeviceCharacteristics(accessory, deviceData)
                .then((a) => {
                    accessory = a;
                    this.loadData(accessory, deviceData)
                        .then((b) => {
                            accessory = b;
                        });
        ...
        //     loadDataFn(deviceData, myObject);
        // };
    ...
    initializeDeviceCharacteristics(accessory, deviceData) {
        ...
        let that = this;
        return new Promise((resolve, reject) => {
            that.deviceGroup = 'unknown'; // that way we can easily tell if we set a deviceData group
            ...
            if (deviceData && deviceData.capabilities) {
                ...
                if (isFan === true && (deviceData.capabilities['Fan Light'] !== undefined || deviceData.capabilities['FanLight'] !== undefined || that.deviceGroup === 'unknown')) {
                ...
                if (deviceData.capabilities['Switch'] !== undefined && (deviceData.capabilities['Fan Light'] !== undefined || deviceData.capabilities['FanLight'] !== undefined || that.deviceGroup === 'unknown')) {
                    //Handles Standalone Fan with no levels
                    if (isLight === true) {
                ...
                if (deviceData.capabilities['Energy Meter'] !== undefined && that.deviceGroup === 'unknown') {
                ...
                if (deviceData.capabilities['Power Meter'] !== undefined && that.deviceGroup === 'unknown') {
