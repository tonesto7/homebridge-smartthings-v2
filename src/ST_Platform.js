...
module.exports = class ST_Platform {
  ...
  didFinishLaunching() {
    this.log(
      `Fetching ${platformName} Devices. NOTICE: This may take a moment if you have a large number of devices being loaded!`
    );
    ...
    this.refreshDevices()
      .then(() => {
        that
          ...
          .then(resp => {
            if (resp.status === "OK") that.client.startDirect(null);
          });
      })
      .catch(err => {
        that.log.error(err);
        ...
        this.client
          ...
          .then(resp => {
            ...
            if (resp && resp.deviceList && resp.deviceList instanceof Array) {
              ...
              const toCreate = this.SmartThingsAccessories.diffAdd(
                resp.deviceList
              );
              const toUpdate = this.SmartThingsAccessories.intersection(
                resp.deviceList
              );
              const toRemove = this.SmartThingsAccessories.diffRemove(
                resp.deviceList
              );
              that.log.warn(
              ...
              that.log.info(
                `Devices to Update: (${Object.keys(toUpdate).length})`
              );
              that.log.info(
            ...
            that.log(
              `Total Device Initialization Process Time: (${Math.round(
                (new Date() - starttime) / 1000
              )} seconds)`
            );
            that.log(
              `Unknown Capabilities: ${JSON.stringify(
                that.unknownCapabilities
              )}`
            );
            that.log(
              `DeviceCache Size: (${
                Object.keys(this.SmartThingsAccessories.getAll()).length
              })`
            );
            resolve(true);
          })
  ...
  addDevice(device) {
    ...
    device.excludedCapabilities = this.excludedCapabilities[
      device.deviceid
    ] || ["None"];
    this.log.debug(
      `Initializing New Device (${device.name} | ${device.deviceid})`
    );
    accessory = this.getNewAccessory(device, new_uuid);
    accessory.reachable = true;
    this.homebridge.registerPlatformAccessories(pluginName, platformName, [
      accessory
    ]);
  ...
  updateDevice(device) {
    ...
    accessory = this.SmartThingsAccessories.loadAccesoryData(
      cacheDevice,
      device
    );
    accessory.reachable = true;
  ...
  removeAccessory(accessory) {
    ...
    if (this.SmartThingsAccessories.remove(accessory)) {
      this.homebridge.unregisterPlatformAccessories(pluginName, platformName, [
        accessory
      ]);
      this.log(
        `Removed: ${accessory.context.name} (${accessory.context.deviceid})`
      );
    }
  }
  ...
  configureAccessory(accessory) {
    ...
    let cachedAccessory = this.SmartThingsAccessories.CreateFromCachedAccessory(
      accessory,
      this
    );
  ...
  processFieldUpdate(change) {
    let attrObj = this.SmartThingsAccessories.getAttributeStoreItem(
      change.attribute,
      change.deviceid
    );
    ...
    if (attrObj instanceof Array) {
      attrObj.forEach(item => {
        if (accessory) {
          accessory.context.deviceData.attributes[change.attribute] =
            change.value;
          item.getValue();
          // this.log(item);
        }
    ...
    return new Promise(resolve => {
      try {
        ...
        webApp.listen(that.configItems.direct_port, () => {
          that.log(
            `Direct Connect is Listening On ${ip}:${that.configItems.direct_port}`
          );
        });
        ...
        webApp.get("/restart", (req, res) => {
          ...
          that.log(
            "Received request from " +
              platformName +
              " to restart homebridge service in (" +
              delay / 1000 +
              " seconds) | NOTICE: If you using PM2 or Systemd the Homebridge Service should start back up"
          );
          setTimeout(() => {
            process.exit(1);
          }, parseInt(delay));
        ...
        webApp.get("/refreshDevices", (req, res) => {
          ...
          that.log(
            "Received request from " + platformName + " to refresh devices"
          );
          that.refreshDevices();
          res.send({
            status: "OK"
        ...
        webApp.post("/updateprefs", (req, res) => {
          ...
          if (
            ...
            that.log(
              platformName +
                " Updated Local Commands Preference | Before: " +
                that.local_commands +
                " | Now: " +
                data.local_commands
            );
          ...
          if (data.local_hub_ip && that.local_hub_ip !== data.local_hub_ip) {
            ...
            that.log(
              platformName +
                " Updated Hub IP Preference | Before: " +
                that.local_hub_ip +
                " | Now: " +
                data.local_hub_ip
            );
          ...
          if (Object.keys(data).length > 3) {
            ...
            that.log(
              ...
              "(" +
                data.change_name +
                ") [" +
                (data.change_attribute
                  ? data.change_attribute.toUpperCase()
                  : "unknown") +
                "] is " +
                data.change_value
            );
